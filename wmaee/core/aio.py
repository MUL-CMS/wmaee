...
async def open_unix_connections(sock_in, sock_out, sock_err, *, loop=None):
    if loop is None:
        loop = asyncio.get_event_loop()
    # Reader
    in_reader, in_writer = await asyncio.open_unix_connection(sock_in, loop=loop)
    out_reader, out_writer = await asyncio.open_unix_connection(sock_out, loop=loop)
    err_reader, err_writer = await asyncio.open_unix_connection(sock_err, loop=loop)
    return in_reader, out_writer, err_writer
